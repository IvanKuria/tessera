...
/// and transient server (5xx) failures.
///
/// Polymarket's Gamma and CLOB APIs send **no** `Retry-After` / `X-RateLimit-*`
/// headers, so retry timing is purely client-driven. Delays follow
/// `base * factor^attempt`, capped at `maxDelay`, with ±`jitterFraction`
/// randomization to avoid thundering-herd retries. The randomness uses
/// `SystemRandomNumberGenerator` (not a `Date()`-seeded RNG), so tests can
